...
exports.storageClient = exports.LocalInfo = void 0;
...
const fileType = (stat) => {
    if (stat.isDirectory()) {
        return mobiletto_base_1.M_DIR;
    }
    if (stat.isFile()) {
        return mobiletto_base_1.M_FILE;
    }
    if (stat.isSymbolicLink()) {
        return mobiletto_base_1.M_LINK;
    }
    return mobiletto_base_1.M_SPECIAL;
};
exports.LocalInfo = {
    driver: "local",
    scope: "local",
...
class StorageClient {
    constructor(baseDir, opts) {
        ...
        this.info = () => (Object.assign({ canonicalName: () => `local:${this.baseDir}` }, exports.LocalInfo));
        ...
        /* eslint-enable @typescript-eslint/no-explicit-any */
        isNotExistError(err)
            ? err instanceof mobiletto_base_1.MobilettoNotFoundError
                ? err
                : new mobiletto_base_1.MobilettoNotFoundError(this.denormalizePath(path))
            : err instanceof mobiletto_base_1.MobilettoError || err instanceof mobiletto_base_1.MobilettoNotFoundError
                ? err
        ...
        this.fileToObject = (dir) => (f) => {
            ...
            if (type === mobiletto_base_1.M_LINK) {
        ...
        this.readDirFiles = (dir, recursive, visitor) => __awaiter(this, void 0, void 0, function* () {
            ...
            for (const file of files) {
                if (file.type === mobiletto_base_1.M_DIR) {
        ...
        this.dirFiles = (dir, visitor) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (isNotExistError(e)) {
                    ...
                    const files = [];
                    try {
                        const parent = (0, path_1.dirname)(norm);
                        const base = (0, path_1.basename)(norm);
                        ...
                        if (isNotExistError(e2)) {
                            mobiletto_base_1.logger.debug(`dirFiles (try-file) error: ${JSON.stringify(e2)}`);
                        }
                        else {
                            mobiletto_base_1.logger.warn(`dirFiles (try-file) error: ${JSON.stringify(e2)}`);
                        }
                    ...
                    if (files.length > 0) {
                        ...
                        mobiletto_base_1.logger.debug(`dirFiles (try-file) not found: ${norm}`);
                        throw this.ioError(e, norm, "dirFiles");
                    }
                }
                mobiletto_base_1.logger.warn(`dirFiles error: ${JSON.stringify(e)}`);
                throw this.ioError(e, norm, "dirFiles");
            }
        });
        if (!baseDir) {
            throw new mobiletto_base_1.MobilettoError("local.StorageClient: key (baseDir) is required");
        }
        ...
        if (!fs.existsSync(baseDir)) {
            if (opts === null || opts === void 0 ? void 0 : opts.createIfNotExist) {
                ...
                throw new mobiletto_base_1.MobilettoError(`local.StorageClient: baseDir does not exist (and opts.createIfNotExist was not true): ${baseDir}`);
            }
        }
        else {
            ...
            if (!resolved.stat.isDirectory()) {
                ...
                throw new mobiletto_base_1.MobilettoError(`local.StorageClient: baseDir is not a directory: ${baseDir}${dest ? ` (resolved to ${dest})` : ""}`);
            }
            dir = resolved.path;
        }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (visitor === null) {
                    ...
                    return yield this.readDirFiles(dir, recursive, visitor);
                }
            }
            catch (err) {
                if (err instanceof mobiletto_base_1.MobilettoNotFoundError && !recursive && pth.includes("/")) {
                    // are we trying to list a single file?
                    const parentDir = this.normalizePath((0, path_1.dirname)(pth));
                    const results = [];
    ...
    metadata(path) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!lstat) {
                throw new mobiletto_base_1.MobilettoError("metadata: lstat error");
            }
            const type = fileType(lstat);
            if (type === mobiletto_base_1.M_DIR && path !== "") {
                let contents;
                try {
                    contents = yield this.list(path);
                }
                catch (err) {
                    throw this.ioError(err, path, "metadata");
                }
    ...
    mkdirs(path) {
        try {
            mobiletto_base_1.logger.debug(`mkdirs: creating directory: ${path}`);
            fs.mkdirSync(path, { recursive: true, mode: this.dirMode });
        }
        catch (err) {
            throw new mobiletto_base_1.MobilettoError(`mkdirs: error creating directory ${path}: ${err}`, err);
        }
    }
    normalizePathAndEnsureParentDirs(path) {
        return __awaiter(this, void 0, void 0, function* () {
            const file = this.normalizePath(path);
            mobiletto_base_1.logger.debug(`read: reading path: ${path} - ${file}`);
            const parent = (0, path_1.dirname)(file);
            let dirStat;
            try {
                ...
                if (isNotExistError(err)) {
                    ...
                    throw new mobiletto_base_1.MobilettoError(`write: lstat error on ${parent}: ${err}`, err);
                }
            }
            if (typeof dirStat === "undefined") {
                ...
                throw new mobiletto_base_1.MobilettoError(`write: not a directory: ${parent} (cannot write file ${file})`);
            }
            return file;
        });
    }
    write(path, generatorOrReadableStream) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            mobiletto_base_1.logger.debug(`write: writing path ${path} -> ${file}`);
            const stream = fs.createWriteStream(file, { mode: parseInt(this.fileMode, 8) });
            const writer = (0, mobiletto_base_1.writeStream)(stream);
            const closer = (0, mobiletto_base_1.closeStream)(stream);
            let count = 0;
    ...
    read(path, callback, endCallback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                return yield (0, mobiletto_base_1.readStream)(stream, callback, endCallback);
            }
            catch (err) {
    ...
    remove(path, optsOrRecursive, quiet) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            mobiletto_base_1.logger.debug(`remove: deleting path: ${path} = ${file}`);
            try {
                ...
                if (isNotExistError(err)) {
                    ...
                    throw new mobiletto_base_1.MobilettoError(`remove(${path}) error: ${err}`, err);
                }
            }
...
const storageClient = (key, secret, opts) => {
    if (!key) {
        throw new mobiletto_base_1.MobilettoError("local.storageClient: key is required");
    }
