...
const SINGLE_DRIVER = "local_1";

const DRIVER_CONFIG = {
    local_1: {
        key: `/tmp/mobiletto-driver-local_test1_`,
        opts: { createIfNotExist: true },
    },
    local_2: {
        key: `/tmp/mobiletto-driver-local_test2_`,
        opts: { createIfNotExist: true },
    },
};
const newTestDir = (config) => `${config.key}_${rand(10)}`;
...
for (const redisSetup of redisTests()) {
    ...
    for (const driverName of DRIVER_NAMES) {
        ...
        const driverType = driverName.substring(0, driverName.indexOf("_"));
        ...
        describe(`${driverTest} test`, () => {
            ...
            describe(`${driverTest} - create api client`, () => {
                it("should validate the config and return an API object", async () => {
                    try {
                        const api = await mobiletto(driverType, newTestDir(config), config.secret, config.opts);
            ...
            describe(`${driverTest} - write a file, read file, read metadata, delete file`, () => {
                ...
                before((done) => {
                    ...
                    connect(driverType, config.key, config.secret, config.opts)
                        .then((api) => {
            ...
            for (const encryption of encryptionTests()) {
                ...
                if (encryption && redisSetup.name !== "redis-enabled" && driverType !== "local") {
                ...
                describe(`${driverTest} - ${encDesc} fail to write and delete files in readOnly mode`, () => {
                    ...
                    before((done) => {
                        ...
                        connect(driverType, config.key, config.secret, opts, encryption)
                            .then((api) => {
                                fixture = { api, name, randomData };
                            })
                            .catch((err) => {
            ...
            for (const encryption of encryptionTests()) {
                ...
                const encDesc = encryption ? "(with encryption)" : "(without encryption)";
                const mirrorCandidates = DRIVER_NAMES.filter((n) => n !== driverName);
                const mirrorDriverName = mirrorCandidates[(mirrorCandidates.length * Math.random()) | 0];
                const mirrorDriver = mirrorDriverName.substring(0, mirrorDriverName.indexOf("_"));
                const mirrorConfig = DRIVER_CONFIG[mirrorDriverName];
                const mirrorDest = `mirrorDest_${rand(5)}/`;
                describe(`${driverTest} - ${encDesc} write files in a new dir, read metadata, mirror to another place (${mirrorDriverName}), verify mirror, recursively delete from both`, () => {
                    ...
                    let fixture;
                    before((done) => {
                        mobiletto(driverType, newTestDir(config), config.secret, config.opts, encryption)
                            .then((api) => {
                    ...
                    it("should successfully recursively list files and see all correct subdirectories and files", async () => {
                        await new Promise((resolve, reject) => {
                            // ensure all writes have been flushed to disk before we try reading again
            ...
            describe(`${driverTest} - write an encrypted file, read file, read metadata, delete file`, () => {
                ...
                before((done) => {
                    const name = `test_file_${fileSuffix}`;
                    mobiletto(driverType, newTestDir(config), config.secret, config.opts, { key: encryptionKey })
                        .then((api) => {
                            fixture = { api, name, randomData };
                        })
                        .catch((err) => {
